...
interface GwPrestasiProps {
  currentUser: Guru;
  showToast: (msg: string, type: 'success' | 'error') => void;
}

export const GwPrestasi: React.FC<GwPrestasiProps> = ({ currentUser, showToast }) => {
  ...
  return (
    <div>
      <ConfirmDialog
        isOpen={!!deleteId}
        message="Hapus data prestasi ini?"
        onConfirm={confirmDelete}
        onCancel={() => setDeleteId(null)}
      />

      <div className="flex justify-between items-center mb-6">
        <h2 className="text-2xl font-bold text-white">Catatan Prestasi</h2>
        <button onClick={() => setIsModalOpen(true)} className="bg-success text-white px-4 py-2 rounded hover:bg-emerald-600 transition">+ Catat Prestasi</button>
      </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-4">
          <div className="bg-gray-800 rounded-lg shadow-lg w-full max-w-md p-6 border border-gray-700">
             ...
             <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div>
                    <label className="block text-sm font-medium text-gray-300">Nama Prestasi</label>
                    <input type="text" required className="mt-1 w-full bg-gray-700 border border-gray-600 rounded p-2 text-white"
                ...
                <div>
                    ...
                    <select className="mt-1 w-full bg-gray-700 border border-gray-600 rounded p-2 text-white"
